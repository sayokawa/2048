...
DOMDisplay.prototype.newTile = function(plan, x, y) {
	...
	let tile = this.tiles[x * plan[0].length + y],
		span,
		val = plan[x][y],
		interval,
		dimension = 0,
...
DOMDisplay.prototype.moveTile = function(plan, srcX, srcY, dstX, dstY) {
	...
	let srcTile = this.tiles[srcX * plan[0].length + srcY],
...
window.onload = function() {
	...
	let resizeHandler = function() {
		let marginTop = (window.innerHeight - game.offsetHeight) / 2;
		if (marginTop > 0) {
	...
	let myGame = new Game(new DOMDisplay());
	myGame.start();
